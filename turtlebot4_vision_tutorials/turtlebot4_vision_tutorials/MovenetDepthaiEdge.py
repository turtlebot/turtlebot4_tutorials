...
class Body:
    def __init__(self, scores=None, keypoints_norm=None, keypoints=None,
                 score_thresh=None, crop_region=None, next_crop_region=None):
        """
        Attributes:
        scores : scores of the keypoints
        keypoints_norm : keypoints normalized ([0,1]) coordinates (x,y) in the square crop region
        ...
        next_crop_region : cropping region calculated from the current body
                           keypoints and that will be used on next frame
        """
        self.scores = scores
...
def find_isp_scale_params(size, is_height=True):
    ...
    This function is useful to work around a bug in depthai where ImageManip is scrambling images
    that have an invalid size
...
class MovenetDepthai:
    ...
    - model: Movenet blob file,
                    - "thunder": default thunder blob file (see variable MOVENET_THUNDER_MODEL),
                    - "lightning": default lightning blob file (see variable
    ...
    - score_thresh : confidence score to determine whether a keypoint prediction is reliable
    ...
    - internal_fps : when using the internal color camera as input source, set its FPS to this
                     value (calling setFps()).
    ...
    def __init__(self,
        ...
        if input_src is None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            if self.crop:
                ...
                width, self.scale_nd = find_isp_scale_params(internal_frame_height * 1920 / 1080,
                                                             is_height=False)
        ...
        # Defines the default crop region (pads the full image from both sides to make it a
        # square image) Used when the algorithm cannot reliably determine the crop region
    ...
    def build_processing_script(self):
        ...
        So we build this code from the content of the file template_processing_script.py
        ...
        code = re.sub('\n\\s*\n', '\n', code)
        # For debuging
        with open("/tmp/tmp_code.py", "w") as file:
            file.write(code)
